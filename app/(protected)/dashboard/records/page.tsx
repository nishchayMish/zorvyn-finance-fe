...
import { useRecords, useSummary, queryKeys } from "@/lib/hooks/use-queries"
...
import {
  PlusIcon, Search, Filter, ChevronDown,
...
export default function RecordsPage() {
  ...
  const { data: recordsData, isLoading: recordsLoading, isFetching } = useRecords(queryParams)
  const { data: summary, isLoading: summaryLoading } = useSummary()

  // Collect categories from fetched records
  React.useEffect(() => {
    if (recordsData?.records?.length) {
      const uniqueCats = Array.from(new Set(recordsData.records.map((r: any) => r.category))).filter(Boolean)
      setCategories(prev => Array.from(new Set([...prev, ...uniqueCats])) as string[])
    }
  }, [recordsData])

  // Reset page when filters change
  React.useEffect(() => { setPage(1) }, [typeFilter, categoryFilter, debouncedSearch])

  const hasActiveFilters = typeFilter !== "all" || categoryFilter !== "all"
  const records = (recordsData as any)?.records ?? []
  const pagination = (recordsData as any)?.pagination
    ...
    <div className="flex flex-col gap-6 min-h-screen bg-zinc-950 text-white">
      ...
      <div className="px-4 lg:px-8">
        {summaryLoading ? (
          ...
          <div className="flex gap-3 flex-wrap">
            <StatCard label="Total Income" value={`₹${summary.totalIncome.toLocaleString()}`} icon={<TrendingUp className="h-5 w-5 text-emerald-400" />} borderColor="border-emerald-500/20" />
            <StatCard label="Total Expense" value={`₹${summary.totalExpense.toLocaleString()}`} icon={<TrendingDown className="h-5 w-5 text-rose-400" />} borderColor="border-rose-500/20" />
            <StatCard label="Net Balance" value={`₹${summary.netBalance.toLocaleString()}`} icon={<IndianRupee className="h-5 w-5 text-violet-400" />} borderColor="border-violet-500/20" />
          </div>
        ) : null}
      </div>
        ...
        <div className="flex items-center gap-2">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className={cn("flex items-center gap-1.5 px-3 py-1.5 h-9 text-sm border rounded-lg bg-zinc-900 border-zinc-800 text-zinc-300 hover:bg-zinc-800 transition-colors", typeFilter !== "all" && "border-violet-500/40 text-violet-300 bg-violet-500/10")}>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <button className={cn("flex items-center gap-1.5 px-3 py-1.5 h-9 text-sm border rounded-lg bg-zinc-900 border-zinc-800 text-zinc-300 hover:bg-zinc-800 transition-colors", categoryFilter !== "all" && "border-violet-500/40 text-violet-300 bg-violet-500/10")}>
            ...
            <DropdownMenuContent className="bg-zinc-900 border-zinc-800 text-zinc-200 max-h-[300px] overflow-auto">
              ...
              {categories.map(cat => (
                <DropdownMenuItem key={cat} onClick={() => setCategoryFilter(cat)} className="cursor-pointer focus:bg-zinc-800 capitalize">{cat}</DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      {/* Table — show skeleton while loading, dim while fetching new page */}
      <div className="px-4 lg:px-8">
        <div className={cn("transition-opacity duration-200", isFetching && !recordsLoading ? "opacity-60" : "opacity-100")}>
          <RecordsTable
            records={records}
